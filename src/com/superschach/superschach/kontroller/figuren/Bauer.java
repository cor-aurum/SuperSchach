...
public class Bauer extends Figur
{
    ...
    public boolean enPassantMoeglich(int x, int y)
    {
        if(((y==2)&&(vorzeichen()==-1))||((y==5)&&(vorzeichen()==1)))
        {
            ...
            if((letzterZug[0]==x)&&(letzterZug[1]==(y+vorzeichen()))&&(letzterZug[3]==(y-vorzeichen())))
            {
                if(((pruefer.diagonal(gebePosX(),gebePosY(),x,y))&&(kontroller.inhalt(x,y-vorzeichen())*vorzeichen()==-8)&&pruefer.diffEins(gebePosX(),gebePosY(),x,y)))
                {
                    return true;
                }
            }
