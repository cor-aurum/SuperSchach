...
public class Koenig extends Figur
{
    ...
    public void zug(int x, int y)
    {
        ...
        else
        {
            if(rochadeMoeglich(x,y))
            {
                /*if(x==gebePosX()+2)
                {
                kontroller.verschiebe(7,y,5,y);
                //kontroller.figur[x+1][y].versetzen(x-1,y);
                }
                else
                {
                kontroller.verschiebe(0,y,3,y);
    ...
    {
        boolean ret=false;
        if((((pruefer.diagonal(gebePosX(),gebePosY(),x,y))||(pruefer.gerade(gebePosX(),gebePosY(),x,y)))&&(kontroller.inhalt(x,y)*vorzeichen()<=0)&&pruefer.diffEins(gebePosX(),gebePosY(),x,y)))
        {
            ret=true;
        }
        return ret;
        ...
        if((!wurdeBewegt())&&(gebePosY()==y))
        {
            ...
            else
            {
                if(x==gebePosX()-2) //Gro�e Rochade 
                {
                    ...
                    {
                        if((kontroller.inhalt(1,y)==0)&&(kontroller.inhalt(2,y)==0)&&(kontroller.inhalt(3,y)==0))
                        {
    ...
    public boolean freiUnbedroht(int von, int biskleiner)
    {
		...
		{
			if (kontroller.inhalt(i, gebePosY()) != 0
					|| pruefer.istBedroht(i, gebePosY()))
			{
				return false;
			}
		}
