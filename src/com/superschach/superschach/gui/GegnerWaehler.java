...
public class GegnerWaehler extends Fenster {
			...
			public void run() {
				try {
					liste.getChildren().add(
							new SpielerButtonBot("Kiana (" + AbstractGUI.meldung("weiss") + ")", 4, "WEISS", true));
					liste.getChildren().add(
							new SpielerButtonBot("Kiana (" + AbstractGUI.meldung("schwarz") + ")", 4, "SCHWARZ", true));
					liste.getChildren().add(new SpielerButtonBot(
							"Ivan Zufallski (" + AbstractGUI.meldung("weiss") + ")", 1, "WEISS", false));
					liste.getChildren().add(new SpielerButtonBot(
							"Ivan Zufallski (" + AbstractGUI.meldung("schwarz") + ")", 1, "SCHWARZ", false));
					liste.getChildren().add(new KeinSpielerButton(AbstractGUI.meldung("keinSpieler")));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private class Detail extends ScrollPane {
		...
		public Detail(String name, long id, String farbe) {
			...
			nameLabel.setStyle("-fx-text-fill:" + farbtemp2 + ";-fx-background-color:" + farbtemp
					+ ";-fx-font-size:20;-fx-font-weight: bold;-fx-padding:10px;");
			nameLabel.setUnderline(true);
			// root.setStyle("-fx-background-color:rgba(0,100,100,0.7);-fx-background-radius:
			// 10;-fx-effect: dropshadow( three-pass-box , rgba(0,0,0,0.6) , 5, 0.0 , 0 , 1
			// );-fx-padding:20px;");
		...
		public void listener(long id, String farbe) {
			try {
				/*
				 * GegnerWaehler.this.hide(); gUI.spiel.laden(sV.getSelected());
				 ...
				 * gUI.feld.entferneFiguren(); gUI.feld.startaufstellung();
				 */
